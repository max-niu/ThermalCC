...
part_A_train = os.path.join(root, 'Train', 'Infrared')
part_A_test = os.path.join(root, 'Test', 'Infrared')



path_sets = [part_A_train, part_A_test]

if not os.path.exists(part_A_train.replace('Infrared', 'gt_fidt_map')):
    os.makedirs(part_A_train.replace('Infrared', 'gt_fidt_map'))

if not os.path.exists(part_A_test.replace('Infrared', 'gt_fidt_map')):
    os.makedirs(part_A_test.replace('Infrared', 'gt_fidt_map'))

if not os.path.exists(part_A_train.replace('Infrared', 'gt_show')):
    os.makedirs(part_A_train.replace('Infrared', 'gt_show'))

if not os.path.exists(part_A_test.replace('Infrared', 'gt_show')):
    os.makedirs(part_A_test.replace('Infrared', 'gt_show'))
...
for img_path in img_paths:
    ...
    with h5py.File(img_path.replace('.jpg', '.h5').replace('Infrared', 'gt_fidt_map'), 'w') as hf:
    ...
    '''for visualization'''
    cv2.imwrite(img_path.replace('Infrared', 'gt_show').replace('jpg', 'jpg'), fidt_map1)
